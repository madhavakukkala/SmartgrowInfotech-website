...
export function AnimatedCard({ className, ...props }: CardProps) {
  return (
    <div
      role="region"
      className={cn(
        "group/animated-card relative w-[356px] max-w-full overflow-hidden rounded-xl border border-border bg-card shadow-sm",
        className,
      )}
      {...props}
    />
...
export function CardVisual({ className, ...props }: CardProps) {
  return (
    <div
      className={cn("h-[180px] w-[356px] max-w-full overflow-hidden", className)}
      {...props}
    />
  );
}

/* --- Visual2: animated donut-chart diagram --------------------------- */

interface Visual2Props {
  mainColor?: string;
  secondaryColor?: string;
  gridColor?: string;
  /** Center label of the donut, e.g. "87%" — animates on hover. */
  restValue?: number;
  hoverValue?: number;
  chips?: string[];
  captionTitle?: string;
...
export function Visual2({
  mainColor = "#1690b8",
  secondaryColor = "#f18a21",
  gridColor = "#80808015",
  restValue = 12.5,
  hoverValue = 87,
  chips = ["ReactJS", "Spring", "Flutter", "AWS", "Python", "Docker"],
  captionTitle = "Live outcome data",
  captionSubtitle = "Hover to see the result.",
}: Visual2Props) {
  ...
  return (
    <>
      ...
      />
      <div className="relative h-[180px] w-[356px] max-w-full overflow-hidden rounded-t-lg">
        <Layer1
          hovered={hovered}
          color={mainColor}
...
const Layer1: React.FC<
  LayerProps & { restValue: number; hoverValue: number }
> = ({ hovered, color, secondaryColor, restValue, hoverValue }) => {
  const [mainProgress, setMainProgress] = useState(restValue);
  ...
  useEffect(() => {
    ...
    if (hovered) {
      timeout = setTimeout(() => {
        setMainProgress(hoverValue);
        setSecondaryProgress(100);
      }, 200);
    } else {
      setMainProgress(restValue);
  ...
  return (
    <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute top-0 left-0 z-[7] flex h-[360px] w-[356px] max-w-full transform items-center justify-center transition-transform duration-500 group-hover/animated-card:-translate-y-[90px] group-hover/animated-card:scale-110">
...
const Layer2: React.FC<{
  ...
  captionTitle: string;
  captionSubtitle: string;
}> = ({ color, captionTitle, captionSubtitle }) => {
  return (
    <div
      className="relative h-full w-[356px] max-w-full"
      style={{ "--color": color } as React.CSSProperties}
    >
      <div className="ease-[cubic-bezier(0.6,0.6,0,1)] absolute inset-0 z-[6] flex w-[356px] max-w-full translate-y-0 items-start justify-center bg-transparent p-4 transition-transform duration-500 group-hover/animated-card:translate-y-full">
        <div className="ease-[cubic-bezier(0.6,0.6,0,1)] rounded-md border border-border bg-background/25 px-2 py-1.5 opacity-100 backdrop-blur-sm transition-opacity duration-500 group-hover/animated-card:opacity-0">
          <div className="flex items-center gap-2">
            ...
            <p className="text-xs text-foreground">{captionTitle}</p>
          </div>
          <p className="mt-1 text-xs text-muted-foreground">{captionSubtitle}</p>
        </div>
      </div>
    </div>
  );
...
const Layer4: React.FC<LayerProps & { chips: string[] }> = ({
  color,
  secondaryColor,
  hovered,
  chips,
  ...
  const positions = [
    ...
    { translateX: "100", translateY: "-50" },
    { translateX: "125", translateY: "0" },
    { translateX: "-125", translateY: "0" },
    { translateX: "-100", translateY: "50" },
    { translateX: "-100", translateY: "-50" },
  ];
  const items = chips.slice(0, 6).map((text, i) => ({ ...positions[i], text }));
